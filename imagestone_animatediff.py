...
class AnimateDiffPipeline(
    ...
    def __init__(
        ...
        print("Successfully initialized ImageStone Pipeline")
        
    def get_sigmas_karras(self, n, sigma_begin, sigma_end, rho=7.0, device="cpu"):
    ...
    def forward_VIIV(
        ...
        callback_on_step_end_tensor_inputs: List[str] = ["latents"],
        decode_chunk_size: int = 16,
        imagestone_interval: List[int] = [0, 60],
        **kwargs,
    ):
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if optim_steps_0 <= i <= optim_steps_1:
                        ...
                        latents_list = torch.split(latents, 1, dim=2)
                        image_latents = [latent.squeeze(2) for latent in latents_list]
                        
                        image_list = torch.split(pred_original_samples, 1, dim=2)
                        clean_image_latents = [image.squeeze(2) for image in image_list]
                        
                        cache_latents = []
                        for clean_image_latent, image_latent in zip(clean_image_latents, image_latents):
                            # TODO Zigzag First Order Optim for image latents
                            new_image_latent = image_latent
                            ...
                            if self.do_classifier_free_guidance:
                                ...
                                image_noise_pred = image_noise_pred_uncond + 4. * (image_noise_pred_text - image_noise_pred_uncond)
                            self.scheduler._step_index = None
                            new_image_latent = self.scheduler.step(image_noise_pred, prev_t, new_image_latent, **extra_step_kwargs).prev_sample
                            
                            # TODO Zigzag Second Order Optim for image latents
                            image_latent_model_input = torch.cat([new_image_latent] * 2) if self.do_classifier_free_guidance else new_image_latent
                            ...
                            clean_image_latent = (new_image_latent - gaussian_noise * ((1 - self.scheduler.alphas_cumprod[prev_t]) ** (0.5))) / ((self.scheduler.alphas_cumprod[prev_t]) ** (0.5))
                            cache_latents.append(new_image_latent.unsqueeze(2))
                        latents = torch.cat(cache_latents, dim=2)
    ...
    def forward_is_plus(
        ...
        decode_chunk_size: int = 16,
        imagestone_interval: List[int] = [0, 60],
        guidance_scale_list: List[float] = [5, 3, 3, 5],
        **kwargs,
    ):
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if optim_steps_0 <= i <= optim_steps_1:
                        ...
                        # ---------------------------------------
                        latents = self.inv_scheduler.step(noise_pred, t, latents)
                        pred_original_samples = latents.pred_original_sample
                        latents = latents.prev_sample
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if optim_steps_0 <= i <= optim_steps_1:
                        ...
                        image_list = torch.split(latents, 1, dim=2)
                        clean_image_latents = [image.squeeze(2) for image in image_list]
